...
- Search Console account recheck: `/sitemap.xml` is `Success`, was last read on September 17, 2026, and contains 37 discovered pages. HTTPS is also green with 18 HTTPS URLs, 0 non-HTTPS URLs, and no critical issues.
- Search Console indexing recheck: the report currently shows 37 indexed pages and 46 not indexed pages: 28 old 404 URLs, 3 alternate pages with proper canonicals, 12 crawled but not indexed, and 3 discovered but not indexed. No current sitemap mismatch was found, so no removal, validation, or indexing request was submitted.
- Search Console experience recheck: Core Web Vitals has insufficient Chrome UX data for both mobile and desktop, so there is no confirmed CWV regression to repair from this report. Breadcrumbs, Image Metadata, Review snippets, and Videos are currently valid.
- Search Console traffic snapshot: the Overview report shows 62 total web search clicks. The report also surfaced rising impressions for the query `photojournalism photographers`, which supports keeping the new photojournalism wording on the federal credit page.
- GA4 account recheck: Realtime is receiving page traffic in the `Jordan Licon Photography - GA4` property, including the homepage, blog archive, and multiple article pages. This confirms pageview receipt; `generate_lead` remains unverified because no test inquiry was submitted.
- Semrush account check: the open comparison and Backlink Gap tabs could not be read through the browser bridge during this run. No Semrush metric is being treated as verified until an export or readable report is available.
- Semrush follow-up: the signed-in Chrome account shows Site Health `99%`, Visibility `100%`, `2` organic keywords, `0` estimated organic traffic, and `366` backlinks for `www.jordanliconphotography.com`. The organic competitor report is gated after 10 free requests, and the API host was not resolvable from this environment, so competitor comparisons and backlink-gap prospects remain unverified. No paid trial or account change was started.
- Ahrefs Web Analytics: installed the provided site data key once in each of the 47 production HTML pages, including `success.html`, and verified the tag in the published GitHub source and the live homepage after the CDN refresh.
- Ahrefs free-plan access: the account is on `Webmaster Tools (Free)`. The verified project exposes Site Audit, Site Explorer, Web Analytics, Bot Analytics, GSC Insights, Rank Tracker, and project settings in the account UI; paid API reports and higher limits are not assumed available.
